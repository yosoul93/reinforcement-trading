...
class Arguments:
    def __init__(self, agent=None, env=None, gpu_id=None, if_on_policy=False):
        ...
        self.num_threads = 3 + 24  # cpu_num for evaluate model, torch.set_num_threads(self.num_threads)
...
def mp_evaluate(args, pipe2_eva):
    ...
    '''init: Evaluator'''
    evaluator = Evaluator(cwd=cwd, agent_id=agent_id, device=torch.device("cuda" if torch.cuda.is_available() else "cpu"), env=env_eval,
                          eval_gap=eval_gap, eval_times1=eval_times1, eval_times2=eval_times2, )  # build Evaluator

    '''act_gpu without gradient for pipe1_eva'''
    # pipe1_eva.send(agent.act)
    act = pipe2_eva.recv()

    act_gpu = deepcopy(act).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))  # for pipe1_eva
    [setattr(param, 'requires_grad', False) for param in act_gpu.parameters()]
    ...
    with torch.no_grad():  # speed up running
        ...
        while if_loop:
            ...
            while pipe2_eva.poll():  # receive the latest object from pipe
                ...
                steps_sum += steps
            act_gpu.load_state_dict(act.state_dict())
            if_solve = evaluator.evaluate_save(act_gpu, steps_sum, obj_a, obj_c)
            '''send'''
            pipe2_eva.send(if_solve)
            # if_solve = pipe1_eva.recv()
